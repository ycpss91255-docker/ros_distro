# ros_distro -- ROS 1 多 distro Docker 環境
...
> **TL;DR** — 一鍵建置 ROS 1 容器化開發環境。單一 Dockerfile + 單一
> `BASE_IMAGE` ARG，build 時切換 Noetic / Kinetic 與 `ros:` (custom，
> headless) / `osrf/ros:` (desktop / desktop-full)。預設為
> `osrf/ros:noetic-desktop-full-focal`。取代既有四個 repo
> (`ros_noetic` / `ros_kinetic` / `osrf_ros_noetic` / `osrf_ros_kinetic`)。
>
> ```bash
> ./build.sh && ./run.sh                                                  # 預設：noetic desktop-full
> ./build.sh --build-arg BASE_IMAGE=ros:noetic-ros-base-focal             # noetic headless
> ./build.sh --build-arg BASE_IMAGE=osrf/ros:kinetic-desktop-full-xenial  # kinetic 含 GUI
> ```
>
> 完整 build target 清單請見英文 README 的 [Build targets](../README.md#build-targets) 一節。
...
- **OSRF desktop-full**：完整 ROS 桌面環境，含 RViz、Gazebo 等
...
> **注意**：此映像使用 `osrf/ros`，僅支援 **x86_64**。如需 ARM/Raspberry Pi 支援，請使用 [ros_noetic](https://github.com/ycpss91255-docker/ros_noetic)。
...
完整開發環境，含 tmux、terminator、vim、git 等。
...
當 subtree 位於 `my_robot_project/docker/osrf_ros_noetic/` 時：

- **IMAGE_NAME**：目錄名為 `osrf_ros_noetic`（非 `docker_*`），偵測會退回到 `.env.example` 讀取 `IMAGE_NAME=osrf_ros_noetic` — 正常運作。
...
| `IMAGE_NAME` | 映像名稱 | `osrf_ros_noetic` |
...
| 優先序 | 規則 | 範例路徑 | 結果 |
|:------:|------|----------|------|
| 1 | 最後一層目錄符合 `docker_*` → 去前綴 | `/home/user/docker_osrf_ros_noetic` | `osrf_ros_noetic` |
| 2 | 掃描路徑（右→左）找 `*_ws` → 取前綴 | `/home/user/ros_noetic_ws/docker_osrf_ros_noetic` | `ros_noetic` |
...
```
/home/user/
├── docker_osrf_ros_noetic/  ← repo（目前目錄）
└── osrf_ros_noetic_ws/      ← 偵測為 WS_PATH
```
...
| `sys` | `osrf/ros:noetic-desktop-full-focal` | OS 基礎：使用者/群組、語系、時區 |
| `base` | `sys` | 通用開發工具（apt） |
