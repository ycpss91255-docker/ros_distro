...
> Dockerfile + 単一の `BASE_IMAGE` ARG で、ビルド時に Noetic / Kinetic と
> `ros:` (custom、ヘッドレス) / `osrf/ros:` (desktop / desktop-full) を
> 切り替え。デフォルトは `osrf/ros:noetic-desktop-full-focal`。既存の
> 4 つの repo (`ros_noetic` / `ros_kinetic` / `osrf_ros_noetic` /
> `osrf_ros_kinetic`) を置き換え。
>
> ```bash
> ./build.sh && ./run.sh                                                  # デフォルト：noetic desktop-full
> ./build.sh --build-arg BASE_IMAGE=ros:noetic-ros-base-focal             # noetic ヘッドレス
> ./build.sh --build-arg BASE_IMAGE=osrf/ros:kinetic-desktop-full-xenial  # kinetic GUI 付き
> ```
>
...
- **OSRF desktop-full**：完全な ROS デスクトップ環境、RViz、Gazebo 等を含む
...
フル機能の開発環境。tmux、terminator、vim、git 等を含む。
...
subtree が `my_robot_project/docker/osrf_ros_noetic/` にある場合：

- **IMAGE_NAME**：ディレクトリ名は `osrf_ros_noetic`（`docker_*` ではない）ため、検出は `.env.example` の `IMAGE_NAME=osrf_ros_noetic` にフォールバック — 正常に動作。
...
| `IMAGE_NAME` | イメージ名 | `osrf_ros_noetic` |
...
| 優先順 | ルール | パス例 | 結果 |
|:------:|--------|--------|------|
| 1 | 最後のディレクトリが `docker_*` に一致 → プレフィックスを除去 | `/home/user/docker_osrf_ros_noetic` | `osrf_ros_noetic` |
| 2 | パスをスキャン（右→左）して `*_ws` を探す → プレフィックスを取得 | `/home/user/ros_noetic_ws/docker_osrf_ros_noetic` | `ros_noetic` |
...
```
/home/user/
├── docker_osrf_ros_noetic/  ← repo（現在のディレクトリ）
└── osrf_ros_noetic_ws/      ← WS_PATH として検出
```
...
| `sys` | `osrf/ros:noetic-desktop-full-focal` | OS 基盤：ユーザー/グループ、ロケール、タイムゾーン |
