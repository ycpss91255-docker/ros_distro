# ros_distro -- ROS 1 多 distro Docker 环境
...
> **TL;DR** — 一键构建 ROS 1 容器化开发环境。单一 Dockerfile + 单一
> `BASE_IMAGE` ARG，build 时切换 Noetic / Kinetic 与 `ros:` (custom，
> headless) / `osrf/ros:` (desktop / desktop-full)。默认为
> `osrf/ros:noetic-desktop-full-focal`。取代既有四个 repo
> (`ros_noetic` / `ros_kinetic` / `osrf_ros_noetic` / `osrf_ros_kinetic`)。
>
> ```bash
> ./build.sh && ./run.sh                                                  # 默认：noetic desktop-full
...
> ```
>
> 完整 build target 清单请见英文 README 的 [Build targets](../README.md#build-targets) 一节。
...
- **OSRF desktop-full**：完整 ROS 桌面环境，含 RViz、Gazebo 等
...
完整开发环境，含 tmux、terminator、vim、git 等。
...
当 subtree 位于 `my_robot_project/docker/osrf_ros_noetic/` 时：

- **IMAGE_NAME**：目录名为 `osrf_ros_noetic`（非 `docker_*`），检测会回退到 `.env.example` 读取 `IMAGE_NAME=osrf_ros_noetic` — 正常工作。
...
| `IMAGE_NAME` | 镜像名称 | `osrf_ros_noetic` |
...
| 优先序 | 规则 | 示例路径 | 结果 |
|:------:|------|----------|------|
| 1 | 最后一层目录符合 `docker_*` → 去前缀 | `/home/user/docker_osrf_ros_noetic` | `osrf_ros_noetic` |
| 2 | 扫描路径（右→左）找 `*_ws` → 取前缀 | `/home/user/ros_noetic_ws/docker_osrf_ros_noetic` | `ros_noetic` |
...
```
/home/user/
├── docker_osrf_ros_noetic/  ← repo（当前目录）
└── osrf_ros_noetic_ws/      ← 检测为 WS_PATH
```
...
| `sys` | `osrf/ros:noetic-desktop-full-focal` | OS 基础：用户/组、语言、时区 |
| `base` | `sys` | 通用开发工具（apt） |
